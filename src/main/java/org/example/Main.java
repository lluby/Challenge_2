...
public class Main {
    ...
    public static final ArrayList<Pesanan> pesananList = new ArrayList<>();

    private static final Pembayaran pembayaran = new Pembayaran(); // Object untuk melakukan pembayaran

    public static void main(String[] args) {
        // Membuat object menu dan menambahkannya ke list
    ...
    public static void tampilkanMenu() { // Membuat method dengan nama tampilkanMenu tanpa parameter
        ...
        for (Menu menu : menuList) { // Memanggil list menu
    ...
    public static void pesanDanBayar() { // Membuat method dengan nama pesanDanBayar tanpa parameter
        ...
        for (Pesanan pesanan : pesananList) { // Memanggil list pesanan
            if (pesanan.getQty() > 0) {
                totalPembelian += pesanan.getQty(); // Menghitung total pembelian
                totalHarga += pesanan.getPrice(); // Menghitung total harga
            }
        }
        ...
        if (totalPembelian == 0) {
            ...
            for (Pesanan pesanan : pesananList) { // Memanggil list pesanan
                if (pesanan.getQty() > 0) {
                    System.out.println(pesanan); // Menampilkan informasi pesanan
                }
            }
            System.out.println("-------------------------------");
